...
public class ShizukuSettings {
    ...
    /** Last wireless ADB TLS/connect port that successfully started Shizuku. */
    public static final String LAST_ADB_WIRELESS_PORT = "last_adb_wireless_port";
    ...
    public static int getLastAdbWirelessPort() {
        return getPreferences().getInt(LAST_ADB_WIRELESS_PORT, -1);
    }

    public static void setLastAdbWirelessPort(int port) {
        if (port > 0 && port <= 65535) {
            getPreferences().edit().putInt(LAST_ADB_WIRELESS_PORT, port).apply();
        }
    }
